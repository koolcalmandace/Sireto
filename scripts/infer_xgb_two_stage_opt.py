...
def _is_public_entity(crm_name, c):
    public_keywords = ["MAIRIE", "COMMUNE", "ECOLE", "COLLEGE", "LYCEE", "CHS", "SDIS", "HOPITAL", "PREFECTURE", "CENTRE HOSPITALIER", "SERVICE TECHNIQUE", "INTERCO", "INTERCOM", "CCAS", "DPE", "SIP", "GCS", "GCSMS"]
...
class OptimizedXgbInferenceEngine(XgbInferenceEngine):
    ...
    def from_profile(cls, profile):
        ...
        engine.database_path = getattr(profile, "database_path", Path("data/sirene_cache.sqlite"))
        engine.disable_scraper = getattr(profile, "disable_scraper", False)
        return engine
    ...
    def _search_web_scraper_siret(self, crm_name: str, city: str, postcode: str) -> str | None:
        ...
        clean_pc = str(postcode or "").strip()
        clean_city = re.sub(r"[^\w\s]", " ", city or "").strip()
        
        # Build 2 queries: first is short/high-relevance, second is full detail (for municipal/commune services)
        queries = [
            f"{crm_name} {clean_pc} siret".strip(),
            f"{crm_name} {clean_city} {clean_pc} siret".strip()
        ]
        ...
        for idx, query in enumerate(queries):
            if idx > 0 and (not clean_city or query == queries[0]):
                continue
                
            url = f"https://search.yahoo.com/search?p={urllib.parse.quote_plus(query)}"
            print(f"[Web Scraper Fallback] Querying search engine ({idx+1}/{len(queries)}): '{query}'...")
            try:
                # Respect search engine by sleeping a tiny bit to avoid rapid fire
                time.sleep(0.5)
                req = urllib.request.Request(url, headers=headers)
                with urllib.request.urlopen(req, timeout=5) as response:
                    html = response.read().decode("utf-8", errors="ignore")
                    
                sirets = re.findall(r"\b\d{14}\b", html)
                sirens = re.findall(r"\b\d{9}\b", html)
                
                valid_sirets = [s for s in sirets if is_luhn_valid(s)]
                valid_sirens = [s for s in sirens if is_luhn_valid(s)]
                
                url_matches = re.findall(r"(?:pappers\.fr|societe\.com|infogreffe\.fr|societe\.ninja|annuaire-entreprises\.data\.gouv\.fr)/[a-zA-Z0-9\-\/]*/(\d{9}|\d{14})", html)
                valid_url_nums = [n for n in url_matches if is_luhn_valid(n)]
                
                candidates = valid_url_nums + valid_sirets + valid_sirens
                ...
                if final_candidates:
                    matched_num = final_candidates[0]
                    print(f"[Web Scraper Fallback] Extracted valid identifier: {matched_num}")
                    return matched_num
                    
            except Exception as e:
                print(f"[Web Scraper Fallback] Query failed: {e}")
    ...
    def _build_candidate_pool(
        ...
        res = (result.candidates, result.idf_map, result.default_idf)
        self._cand_pool_cache[cache_key] = res
    ...
    def _search_sqlite_candidates(self, crm_input, postcode: str | None) -> List[dict]:
        ...
        forms = [r"\bSAS\b", r"\bSARL\b", r"\bEURL\b", r"\bSA\b", r"\bSCI\b", r"\bSNC\b", r"\bEI\b", r"\bEIRL\b"]
        for f in forms:
        ...
        prefixes = []
        # Keep prefix search based on original clean words to avoid massive prefix terms
        ...
        def run_search(postcode_val, dept_val):
            # 1. Prefix search
            ...
            # 2. Contains search (Only at postcode level for speed and accuracy)
        ...
        rows = []
        if postcode:
            rows = run_search(postcode_val=postcode, dept_val=None)
        if not rows and dept:
            rows = run_search(postcode_val=None, dept_val=dept)
        if not rows:
            rows = run_search(postcode_val=None, dept_val=None)
        ...
        for state in batch_state:
            ...
            for idx_c, (siret, _) in enumerate(cand_list_n):
                ...
                is_address_perfect = (
                    ...
                    (city_match >= 0.5 or postcode_match >= 0.5)
                )
                if is_address_perfect:
                    ...
                    # Guard: Must have some name similarity/evidence to avoid boosting unrelated companies on the same street
                    has_name_evidence = (
                ...
                # Rule 2: High name similarity + matching street (resolves minor entrance numbers typos)
                is_high_name_sim = (
                    name_jaro_max >= 0.85 and 
                    postcode_match >= 0.5 and 
                    street_name_jaro >= 0.95 and 
                    street_num_diff <= 10
                )
                if is_high_name_sim:
                ...
                # Rule 2B: Modest name similarity (>=0.80) + perfect address match (resolves branch acronym name mismatches)
                ...
                # Rule 3: Near-perfect name match + matching city/postcode (handles street name updates or relocations)
                is_perfect_name_city = (
                    name_jaro_max >= 0.98 and 
                    (city_match >= 0.5 or postcode_match >= 0.5)
                )
