...
def _extract_core_company_name(crm_name: str) -> str:
    if not crm_name:
        return ""
    import re
    parts = re.split(r"\s+[-/:–]\s+|\s*\(", crm_name)
        ...
        for state in batch_state:
            ...
            for idx in top_n_idx:
                ...
                if p_name and p_name.upper().strip() in gen_enseignes:
                    ...
                    # Recalculate Jaro-Winkler against full parent legal name
                    from rapidfuzz.distance import JaroWinkler
                    crm_norm = crm_pre.get("crm_name", "").upper()
                    if crm_norm and p_name:
                        feat["name_jaro_max"] = max(feat.get("name_jaro_max", 0.0), float(JaroWinkler.similarity(crm_norm, p_name.upper())))
