...
def _prepare_top1(df: pd.DataFrame) -> pd.DataFrame:
    """Prepares top1 predictions and computes score_gap."""
    r1 = df[df["rank"] == 1].copy()
    r2 = df[df["rank"] == 2].copy()
    
    r2_scores = r2.set_index("crm_id")["score"]
    r1["score_top2"] = r1["crm_id"].map(r2_scores).fillna(0.0)
    r1["score_gap"] = r1["score"] - r1["score_top2"]
    r1 = r1.drop(columns=["score_top2"])
    return r1
...
def merge_with_ground_truth(infer_df: pd.DataFrame, gt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "rank" in infer_df.columns:
        top1 = _prepare_top1(infer_df)
    else:
        top1 = infer_df.sort_values("score", ascending=False).groupby("crm_id").first().reset_index()
        top1["score_gap"] = 0.0  # Heuristic fallback if no ranks are present
