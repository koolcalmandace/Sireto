"""Two-Stage XGBoost Inference Engine — Version 3.5 (Full Opt 3.5 SSOT)
...
5. Active Sister-Branch Disambiguation Engine (_resolve_active_sister_branches):
...
6. Calibrated Threshold & Generic Query Safety Floor:
...
# ---------------------------------------------------------------------------
# Version 3.5 Engine Class (SSOT Baseline)
# ---------------------------------------------------------------------------

class Version35XgbInferenceEngine(XgbInferenceEngine):
    """Version 3.5 Two-Stage XGBoost Engine (V3.0 SSOT Core + Robust Keys + Sister Tie-Breaker)."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.auto_threshold = 0.85  # Calibrated V3.5 AUTO threshold
    ...
    def _resolve_active_sister_branches(
        ...
        # Tier 2: Sub-Entity / Location Descriptor Token Overlap (Fixed: Uses crm_query_name parameter)
        crm_name_upper = str(crm_query_name or "").upper().strip()
        keywords = ["CENTRE", "GARE", "NORD", "SUD", "EST", "WEST", "PARC", "ECLUSES", "AGENCE", "BOURSE", "SITE", "POLE"]
        matched_kws = [kw for kw in keywords if kw in crm_name_upper]
    ...
    def process_row(
        ...
        """Process a single CRM record through Version 3.5 Pipeline."""
...
# ---------------------------------------------------------------------------
# Main Execution Entrypoint (V2.0-V2.6 JSONL Architecture + V3.5 SSOT)
# ---------------------------------------------------------------------------

def main():
    parser = argparse.ArgumentParser(description="Version 3.5 Two-Stage XGBoost Engine (V3.0 SSOT + Robust Keys + Sister Tie-Breaker)")
    ...
    logging.info("Initializing Version 3.5 Engine (V3.0 SSOT Core + Robust Column Keys + JSONL Blueprint)...")
    ...
    engine = Version35XgbInferenceEngine.from_profile(profile)
    ...
    progress = tqdm(total=total_queries, initial=len(completed_orig_indices), desc="Matching V3.5 SSOT")
    batch_buffer = []
    for idx, row in df.iterrows():
        ...
        row_res["v35_siret"] = res.get("siret")
        row_res["v35_score"] = res.get("score")
        row_res["v35_decision"] = res.get("decision")
        if res.get("top_match"):
            row_res["v35_match_name"] = res["top_match"].get("denomination")
            row_res["v35_match_city"] = res["top_match"].get("city")

        results.append(row_res)
        completed_orig_indices.add(orig_index)
        batch_buffer.append(row_res)

        # Write progress buffer in chunks of 500 to eliminate disk write overhead (True V2.6 chunking)
        if len(batch_buffer) >= 500:
            try:
                ...
                logging.error(f"Failed to write progress batch to {temp_path}: {e}")
    ...
    # Flush any remaining items in the buffer at the end of execution
    if batch_buffer:
        try:
            with open(temp_path, "a", encoding="utf-8") as f:
                for item in batch_buffer:
            ...
            logging.error(f"Failed to flush final progress batch to {temp_path}: {e}")
    ...
    logging.info(f"Version 3.5 Processing Complete! Saved output to: {out_path}")
