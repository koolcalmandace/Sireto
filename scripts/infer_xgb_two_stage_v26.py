...
def _strip_city_branch_suffix(crm_name: str, crm_city: str = "", postcode: str = "", crm_address: str = "") -> str:
    ...
    if " - " in cleaned or " / " in cleaned:
        ...
        if len(parts) >= 2:
            ...
            # If trailing token matches resolved town or is >= 3 chars city token
            if len(leading) >= 3 and (trailing == resolved_town or (len(trailing) >= 3 and resolved_town and trailing in resolved_town)):
                cleaned = leading
                
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    
    # 4. Minimum 3-Character Safety Guard
    if len(cleaned) < 3:
        return crm_name.upper().strip()
    ...
    def infer_topk_batch(
        ...
        for state in batch_state:
            ...
            for idx in top_n_idx:
                ...
                if crm_norm_eval:
                    for ens_key in ["enseigne1", "enseigne2", "enseigne3", "denomination_usuelle_etablissement", "denomination_usuelle_unite_legale", "denomination_ul", "denomination"]:
