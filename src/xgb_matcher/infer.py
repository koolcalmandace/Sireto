...
class XgbInferenceEngine:
    ...
    def _build_candidate_pool(
        ...
        # Fallback: use unified retrieval (V7/V8b-compatible with strict geo-resolution)
        ...
        crm_row_with_id = dict(crm_row)
        if "crm_id" not in crm_row_with_id:
            crm_row_with_id["crm_id"] = crm_id

        result = build_candidate_pool(
            store=self.store,
            crm_row=crm_row_with_id,
            crm_pre=crm_pre,
            config=config,
            tfidf_cache={},
            gt_siret=None,
